...
cb_fn = os.path.join(main_dir,'s03_counterbalance','task-*_ver*_ses*_block*.csv')
cb_list = glob.glob(cb_fn)
# for loop
# load csv file
output = pd.DataFrame()
output = pd.DataFrame(0, index=np.arange(len(cb_list)), columns=['sum_ITI','sum_ISI1', 'sum_ISI2', 'sum_ISI3'])
output.columns = ['sum_ITI','sum_ISI1', 'sum_ISI2', 'sum_ISI3']
for ind, cb_filename in enumerate(cb_list):
    basename = ntpath.basename(cb_filename)
    p = pd.read_csv(cb_filename)
    output.loc[ind,'sum_ITI']  = np.sum(p['ITI'])
    output.loc[ind,'sum_ISI1']  = np.sum(p['ISI1'])
    output.loc[ind,'sum_ISI2']  = np.sum(p['ISI2'])
    output.loc[ind,'sum_ISI3']  = np.sum(p['ISI3'])

output_filename = os.path.join(main_dir, 's05_checkjitter', 'total-jitter_task-PVC_counterbalance.csv')
