...
for k = 1:nISIs
    for i = 1:iter
        [meanrecipvif(i, k), vifs, design_struct] = s121902_generate_jittered_er_design('noplot', 'ISImean', ISImeans(k));%, 'ISI1mean', ISI1means(k), 'ISI2mean', ISI2means(k));
...
%         [meanrecipvif(i, 1), vifs, design_struct] = generate_jittered_er_design_HJ('noplot', 'ISImean', ISImean);
        [meanrecipvif(i, 1), vifs, design_struct] = s121902_generate_jittered_er_design('noplot', 'ISI1mean', ISI1mean);
...
diaryname = sprintf(['/Users/h/Documents/projects_local/social_influence/design/1222_jitter_type2/social_inf_Events_best_design_of_10000_ver-', num2str(ver,'%03.f'), '.txt'] );
diary(diaryname)
print_matrix(best_design_struct.eventlist, best_design_struct.eventlist_names);
diary off

diaryname = sprintf(['/Users/h/Documents/projects_local/social_influence/design/1222_jitter_type2/social_inf_Events_best_design_of_10000_under_ideal_length_sec_ver-', num2str(ver,'%03.f'),'.txt'] );
diary(diaryname)
