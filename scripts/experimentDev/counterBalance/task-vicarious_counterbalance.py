...
num_select = []
num_select = random.sample(range(1,26),  14)
...
# 2. create administer_items list ________________________________________________________
df['administer_items'] = np.repeat([50, 100],14) # add column 50/100
...
df['trial_type'] = ((df.cue_type == 'low') & (df.administer_items == 50)).astype(int) * 1 + \
    ((df.cue_type == 'high') & (df.administer_items == 50)).astype(int) * 2 + \
    ((df.cue_type == 'low') & (df.administer_items == 100)).astype(int) * 3 + \
    ((df.cue_type == 'high') & (df.administer_items == 100)).astype(int) * 4
    ...
    for index, df_subset in enumerate([df_subset1, df_subset2]):
        ...
        for ind_img, row in cB.iterrows():
            if cB.loc[ind_img,'match'] == "different":
                image_filepath = str(cB.loc[ind_img,'stimuli_num']) + '_' + str(cB.loc[ind_img,'administer_items']) + '_R.jpg'
                cB.loc[ind_img,'image_filename'] = image_filepath
                    # row.imageFileName = imageFilePath
            elif cB.loc[ind_img,'match'] == "same":
                image_filepath = str(cB.loc[ind_img,'stimuli_num']) + '_' + str(cB.loc[ind_img,'administer_items']) + '.jpg'
                cB.loc[ind_img,'image_filename'] = image_filepath
