...
vnames = {'src_subject_id', 'session_id', 'param_runNum','param_counterbalanceVer',...
    'param_counterbalanceBlockNum','param_cue_type','param_administer_type',...
...
if biopac
    ppdev_mex('Open', 1);
    ppdev_mex('Write', 1, 1); % (command, port, value)
    T.param_start_biopac(:) = GetSecs;
    ppdev_mex('Close', 1);
end
...
for trl = 1:size(countBalMat,1)
    ...
    if biopac
        ppdev_mex('Open', 1);
        ppdev_mex('Write', 1, 10); % (command, port, value)
        T.event01_fixation_biopac(trl) = GetSecs;
        ppdev_mex('Close', 1);
    end
    WaitSecs(jitter1);
    fEnd1 = GetSecs;
    ...
    if biopac
        ppdev_mex('Open', 1);
        ppdev_mex('Write', 1, 21); % (command, port, value)
        T.event02_cue_biopac(trl) = GetSecs;
        ppdev_mex('Close', 1);
    end
    WaitSecs(1.00);
    ...
    if biopac
        ppdev_mex('Open', 1);
        ppdev_mex('Write', 1, 31); % (command, port, value)
        T.event03_rating_biopac(trl) = GetSecs;
        ppdev_mex('Close', 1);
    end
    ...
    if biopac
        ppdev_mex('Open', 1);
        ppdev_mex('Write', 1, 41); % (command, port, value)
        T.event04_fixation_biopac(trl) = GetSecs;
        ppdev_mex('Close', 1);
    end
    WaitSecs(jitter2);
    fEnd2 = GetSecs;
    ...
    if biopac
        ppdev_mex('Open', 1);
        ppdev_mex('Write', 1, 51); % (command, port, value)
        T.event05_stimulus_biopac(trl) = GetSecs;
        ppdev_mex('Close', 1);
    end
    ...
    if biopac
        ppdev_mex('Open', 1);
        ppdev_mex('Write', 1, 61); % (command, port, value)
        T.event06_actual_biopac(trl) = GetSecs;
        ppdev_mex('Close', 1);
    end
...
if biopac
    ppdev_mex('Open', 1);
    ppdev_mex('Write', 1, 71); % (command, port, value)
    T.param_end_biopac(trl) = GetSecs;
    ppdev_mex('Close', 1);
end
